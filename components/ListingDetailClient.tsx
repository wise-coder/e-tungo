...
export default function ListingDetailClient({ initialListing }: { initialListing: Listing | null }) {
  ...
  const [fetchedListing, setFetchedListing] = useState<Listing | null>(initialListing);
  ...
  useEffect(() => {
    if (listings.some(item => item.id === id) || fetchedListing?.id === id) { setLoadingListing(false); return; }
    let active = true;
  ...
  const similarListings = listing.status === "sold"
    ? listings
        ...
        <div className="relative h-72 md:h-96">
          {listing.images[activeImage] ? (
            <Image
              ...
              alt={listingImageAlt(listing, activeImage + 1)}
              fill
          ...
          <div className="flex gap-2 px-4 py-2 bg-white overflow-x-auto">
            {listing.images.map((img, i) => (
              ...
              >
                <Image
                  src={img}
                  alt={listingImageAlt(listing, i + 1)}
                  fill
                  className="object-cover"
                  sizes="64px"
                />
      ...
      <div className="px-4 py-5 space-y-5">
        ...
        {listing.status === "sold" && (
          <section className="rounded-2xl border border-gray-200 bg-gray-50 p-4">
            <h2 className="text-lg font-bold text-gray-900">This listing has been sold</h2>
            <p className="mt-1 text-sm text-gray-600">
              Browse similar active listings currently available on e-tungo.
            </p>
            {similarListings.length > 0 ? (
              ...
              <Link href={`/category/${listing.category}`} className="mt-4 inline-block text-sm font-semibold text-brand-700 hover:underline">
                View {getCategoryLabel(listing.category, lang)} listings
              </Link>
            )}
          </section>
        )}
